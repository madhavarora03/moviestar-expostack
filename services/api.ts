import { MediaType, TrendingResult } from '~/interfaces/api-results';
...
export const getSearchResults = async (query: string): Promise<TrendingResult> => {
  ...
  const response = await fetch(
    `https://api.themoviedb.org/3/search/collection?language=en-US&api_key=${API_KEY}&query=${encodeURIComponent(query)}`
  );
